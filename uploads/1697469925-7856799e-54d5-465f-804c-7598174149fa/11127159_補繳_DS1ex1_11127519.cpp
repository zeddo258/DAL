...
bool missionThree( vecString &strMap, int &steps );

bool solveMissionThree( vecString &strMap, int x, int y, int direction, int &pathNum ) ;
...
int main() {
    ...
    while ( command != 0 ) {
        ...
        if ( theresFile ) {
            ...
            else if ( command == 3 ) {                  //任務三
                int steps = 0;
                if ( missionThree( stringOfLine, steps ) ) {
                    printMap( stringOfLine ) ;
                    cout << endl ;
                    printMapForR( stringOfLine ) ;
                    cout << endl << endl << "Shortest path length = " << steps << endl ;
                    cout << "請按任意鍵繼續 . . . " ;
                    char temp;
                    cin >> temp ;
...
int makeCommand( char ch ) {
    ...
    if ( ch != '0' && ch != '1' && ch != '2' && ch != '3' ) {                    //確認是否為0,1,2,3
...
bool missionThree( vecString &strMap, int &steps ) {
    ...
    if ( solveMissionThree( strMap, 0, 0, 0, pathNum ) ) {
...
//        cout << pathX[pathNum] << endl << pathY[pathNum] << endl;
//        cout << strMap[pathX[pathNum]][pathY[pathNum]] << endl;
        pathNum--;
        ...
        while ( pathNum >= 0 ) {
            ...
            if ( solveMissionThree( testMap, shortestX[pathNum], shortestY[pathNum], 0, temp ) ) {


                if ( temp + pathNum + 1 < steps && temp != 0 ) {

                    for ( i = pathNum ; i < 100 ; i++ ) {
                        shortestX[i] = pathX[i] ;
                        shortestY[i] = pathY[i] ;
                    } // for

                    steps = temp + pathNum + 1;
                    vecString tempMap = strMap;
                    strMap = testMap;
                    testMap = tempMap;
                } // if
...
bool solveMissionThree( vecString &strMap, int x, int y, int direction, int &pathNum ) {
    ...
    for ( int i = 0; i < 4; i++ ) {                 //上下左右方向確認
        ...
        if ( isMove( strMap, nextX, nextY ) ) {

            pathX[pathNum] = nextX ;
            pathY[pathNum] = nextY ;
            pathNum++ ;

            if ( solveMissionThree( strMap, nextX, nextY, ( direction + i ) % 4, pathNum ) )
                return true;

...
public:
	...
	void clr(void);
	void ShowshortPassCount(void);
};
void Maze::ShowshortPassCount()
{
    int count = 0 , i , j;
    for (i = 0; i < N; i++)
    {
    ...
    count = count + goal_count;
    cout<<endl<<"Shortest path length :"<<count;
}
void Maze::show()
{
	int i, j;
	...
	if (goal_count != 0)
	{
		for (i = 0; i < N; i++)
		{
			for (j = 0; j < M - 1; j++)
			{
				...
				else if (mazeArray[i][j] == 'P') cout << 'V';
		...
		for (i = 0; i < N; i++)
		{
			...
			{
				if (orignal_maze[i][j] == 'G') cout << orignal_maze[i][j];
				else if (mazeArray[i][j] == 'P') cout << 'E';

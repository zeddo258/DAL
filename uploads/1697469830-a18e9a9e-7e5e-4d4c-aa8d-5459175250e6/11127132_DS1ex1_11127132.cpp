...
class Maze{ 
    ...
    public:
        vector<vector<char>> enter, passed, success; // vector of the input, visited cells, and path.
        vector<vector<CoXY>> path; // vector of the successful path
        vector<CoXY> tmpP; // vector of the coordinate of the successful path
    ...
    do{
        ...
        if((command == 1) || (command == 2)){
            ...
            if(maze.load(fileName, mode)){ // load the original matrix
                ...
                bool suc = maze.findGoals(0,0); // find a pass to goal(s)
                // cerr << "copied enter\n";
                mode = oneG; // reset to default
                maze.show(); // show on screen the visited cells
                if( suc ){ // to check if the goal is reached or not

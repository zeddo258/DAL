...
struct Block {
	...
	bool Rsign = false ; // 是否為最短路徑 
	int step = 0 ; // 儲存此路徑離起點的最短步數 
};
...
public:
	...
	bool findShortestPath( int x, int y ) {
		...
		// 避免找到shortest後，去找多餘的step(意指step>=shortest的情況) 
		if ( step >= shortest ) {
		    step = step - 1 ;
			return false ;
		} // if ( step > shortest )
		...
		// 若此pos是目標G，Gfounded設置為有找到G，並檢查目前的step有沒有比之前的shortest小，有則step取代shortest 
		if ( maze[x][y].pos == 'G' ) {
			Gfounded = true ;
		...
		// 若右下左上四個方位的前方都找過shortest path了，就返回上一步檢查上一步的其他方向 
		step = step - 1 ;
		return false ;
	...
	bool SetShortestPath( int x, int y, int path ) {
		...
		// 避免找到shortest後，去找多餘的step(意指step>=shortest的情況且此格不會是最短路徑) 
		if ( step >= shortest ) {
		    step = step - 1 ;
		...
		if ( maze[x][y].pos == 'G' ) {
			...
			if ( shortest > step ) {
				...
				// 若path等於第一個shortest(第一個找到的G)即印出該最短路徑 
    ...
    if ( (command == 1) || ( command == 2) || ( command == 3 ) ) {
    	...
    	if ( aMaze.load( fileName ) ) {
			...
			// 找出G最短路徑的處理 
				...
				// 若有找到G(表示至少有一條最短路徑)，即印出找到的第一條最短路徑 

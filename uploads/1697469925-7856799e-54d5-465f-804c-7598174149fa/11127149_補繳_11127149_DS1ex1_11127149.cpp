...
enum Mode {aGoal, nGoal, sPath};                                       // 兩種模式: { 找一個終點, 找多個終點 }
...
class Maze
{
    ...
    public:
        ...
        Path findSpath( Location, Path, int& );                 // 找最短路徑
...
bool Maze::InputMaze( string filename, Mode tMode )             // 讀入迷宮資料並儲存,成功回傳true,失敗輸出錯誤資訊並回傳false
{
    ...
    else if ( tMode == sPath )                                          // 確認現在模式是否為 最短路徑終點  
        fsp = true;                                                         // 是,
...
bool Maze::FindGoal( Maze& vMaze )      // // 尋找終點.成功回傳true,失敗回傳false
{
    ...
    if ( vMaze.maze[nL[0]][nL[1]] == 'O' )                              // 確認位置是否為障礙物'O'
        return false;                                                       // 是,回傳false
    ...
    if ( vMaze.maze[nL[0]][nL[1]] == 'G' )                              // 確認vMaze現在位置在迷宮陣列上是否為'G'(終點)
    {
        ...
        if ( j == vMaze.alfgoal.size() )                                    // j等於(vMaze已找到的終點位置鎮列長度(沒找過))
        {
            ...
            if ( fsp )                                                          // 確認是否為模式"3"
            {
                vMaze.location = location;                                      // vMaze位置初始化
                vMaze.direction = Right;                                        // vMaze偵測方向初始化

                Path pt = {};                                                  // 路徑陣列初始化
                int sl = vMaze.path.size();                                     // 最短路徑長度預設為vMaze最快找到終點的的路徑的陣列長度
                Path spath = vMaze.findSpath( vMaze.location, pt, sl );        // 找最短路徑

                if ( sl != vMaze.path.size() )                                  // 確認是否找到更短的路徑
                    vMaze.path = spath;                                         // vMaze最快找到終點的的路徑的陣列長度設為最短路徑陣列
            }
...
Path Maze::findSpath( Location l, Path p, int& sl )
{
    p.push_back( l );                           // 儲存現在位置

    Path ShortP = {}, NewP = {};                // 初始化最短路徑陣列、新路徑陣列

    if ( p.size() == sl )                       // 是否等同已找到最短路徑長度
        return {};                                  // 回傳空陣列


    if ( maze[l[0]][l[1]] == 'E' )              // 是否為未經路徑'E'
        maze[l[0]][l[1]] = 'V';                     // 覆蓋為路過'V'

    if ( maze[l[0]][l[1]] == 'G' )              // 是否為終點'G'
        return p;                                   // 回傳路徑

    Location l2 = {0,0}, Next = {0,0};          // 初始化下一位置、移動數值
    bool ch = true;                             // 初始化是否有路

    for ( int i = 0; i < 4; i++ )
    {
        Next = way();                           // 取出移動數值

        l2[0] = l[0] + Next[0];                 // 設定下一位置Y軸
        l2[1] = l[1] + Next[1];                 // 設定下一位置X軸


        if ( l2[0] + 1 < 1 || l2[0] + 1 > scope[0] || l2[1] + 1 < 1 || l2[1] + 1 > scope[1] )   // 確定下一位置是否超出範圍
            ch = false;                                                                             // 是,是否有路設為否
        else if ( maze[l2[0]][l2[1]] == 'O' )                                                   // 確定下一位置是否為障礙物'O'
            ch = false;                                                                             // 是,是否有路設為否
        else 
        {
            for ( int j = 0; j < p.size(); j++ )
            {
                if ( l2[0] == p[j][0] && l2[1] == p[j][1] )                                     // 確認這條路是否走過這個下一位置
                    ch = false;                                                                     // 是,是否有路設為否
            }
        }
        ...
        if ( ch )                                                                               // 確認是否有路
        {
            NewP = findSpath( l2, p, sl );                                                      // 找出新路徑
            
            if ( NewP.size() != 0 && ( ShortP.empty() || NewP.size() < ShortP.size() ) )        // 確認是否找到路徑且該路徑是否更短
            {    
                ShortP = NewP;                                                                  // 將新路徑設為最短路徑
                sl = ShortP.size();                                                             // 設定最短路徑長度
            }
        }
...
void Maze::printspl()                   // 輸出最短路徑步數
{
    cout << endl << "Short path length : " << path.size();      // 輸出最快找到終點的的路徑長度
    cout << endl;
}
...
int main()              // 主程式
{
    ...
    do
    {
        ...
        cout << endl << "* 3. Short Path      *";
        cout << endl << "**********************";
        cout << endl << "Input a command(0, 1, 2, 3): ";

        cin >> command;             //輸入指令 
        
        if ( ( strcmp( command.c_str(), "1" ) == 0 ) || ( strcmp( command.c_str(), "2" ) == 0 ) || ( strcmp( command.c_str(), "3" ) == 0 ) )   // 確認指令是否為"1"或"2"
        {
            if ( ( strcmp( command.c_str(), "2" ) == 0 ) )                                              // 是,確認指令是否為"2"
                mode = nGoal;                                                                               // 模式設定為找多個終點
            else if( ( strcmp( command.c_str(), "3" ) == 0 ) )                                          // 否,確認指令是否為"3"
                mode = sPath;                                                                              // 模式設定為找最短路徑終點
            ...
            if ( aMaze.InputMaze( fileName, mode ) )                                                    // 確認讀入迷宮資料並儲存是否成功
            {
                ...
                {                                                                                
                    ...
                    if ( ( strcmp( command.c_str(), "3" ) == 0 ) )                                              // 是,確認指令是否為"3"
                        vMaze.printspl();                                                                           // 輸出最短路徑步數
                }

...
enum Modes{oneG, moreG, shortestPath} ; // eNumber used to identify different modes
...
class Maze { // a class type to represent the maze
	...
	public :
		...
		int getDistance() ; // a getter for maze's distance
		void resetDistance() ; // a setter for reset maze's distance
		...
		void findShortestPath(Maze & vMaze, int x, int y, int lastDir, int distance) ; // the function for find a shortest path to goal
	...
	private :
		...
		int theShortestPathDistance ; // the shortest path's distance
...
int Maze::getDistance() { // a getter for maze's distance

	return theShortestPathDistance ;

} // getDistance() ;

void Maze::resetDistance() { // a setter for reset maze's distance

	theShortestPathDistance = 0 ;
...
void Maze::findShortestPath(Maze & vMaze, int x, int y, int lastDir, int distance) { // the function for find a shortest path to goal

  if (!whetherMove(x, y)) { // determine whether the rat can move
      return ;
  } // if
  
  if (theShortestPathDistance != 0 && distance >= theShortestPathDistance) { // determine whether the current distance more than temporary
  	return ; // the current distance bigger than temporary shortest path's distance, return
  } // if
	...
	if (temp[x][y] == 'G') { // determine whether find the goal
		...
		if (distance < theShortestPathDistance || theShortestPathDistance == 0) {
			theShortestPathDistance = distance ; // update the new shortest distance
			theFinalMaze = theMaze ; // update the new shortest path
		} // if
		
		return ; // find G, return
	} // if
  ...
  int arrayOfAllDirCol[] = {1, 0, -1, 0} ; // array used to record all directions {right, down, left, up}
    ...
    nextCol = y + arrayOfAllDirCol[nextDir] ; // the same or next column
		...
		if (whetherMove(nextRow, nextCol) /*&& temp[nextRow][nextCol] != 'V'*/) { // whether the rat can move in the next direction
			findShortestPath(vMaze, nextRow, nextCol, nextDir, distance+1) ; // go to the next direction by recursion
		} // if
		
	} // for
	
	theMaze[x][y] = originalMaze[x][y] ; // restore the maze to original state
  return ; // done all thing, return
...
int main(void) { // the main function
	...
	do {
		...
		int distance = 1 ; // the number of the path to the goal
		...
		if ((command == 1) || (command == 2) || (command == 3)) {
			...
			else if (command == 3) { // command 3 : the shortest path
				mode = shortestPath ;
			} // else if
			...
			if (aMaze.load(fileNumber, mode, goalsTotalAmount)) { // the function for load the original matrix
				...
				if (mode == oneG) { // one goal mode
				...
				else if (mode == moreG) { // more goals mode
					...
					aMaze.findShortestPath(vMaze, 1, 1, 0, distance) ; // the function for find a shortest path to goal
				} // else
				...
				if (aMaze.getDistance() != 0 && mode == shortestPath) { // when mode is find the shortest path and the shortest distance exist
					aMaze.showTheFinalPath() ; // show out the final path
					cout << "Shortest path length = " << aMaze.getDistance() << endl ;
				} // if
				else if (mode == shortestPath && aMaze.getDistance() == 0) { // when mode is find the shortest path and the shortest distance not exist
			...
			aMaze.resetDistance() ; // a setter for reset maze's distance
